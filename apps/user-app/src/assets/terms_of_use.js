export const termsOfUse = `
TERMOS DE USO
Tem Aki no Bairro

Última atualização: ${new Date().toLocaleDateString('pt-BR')}

1. ACEITAÇÃO DOS TERMOS

Ao baixar, instalar ou usar o aplicativo "Tem Aki no Bairro", você concorda em cumprir e estar vinculado a estes Termos de Uso. Se você não concordar com qualquer parte destes termos, não use o aplicativo.

2. DESCRIÇÃO DO SERVIÇO

2.1 Finalidade
O "Tem Aki no Bairro" é um aplicativo de diretório local que permite:
• Visualizar negócios organizados por categorias
• Buscar estabelecimentos por tipo de serviço
• Acessar informações de contato e localização
• Cadastrar novos negócios locais

2.2 Natureza do Serviço
• Aplicativo gratuito para uso pessoal
• Funciona offline com dados armazenados localmente
• Não requer cadastro de usuário
• Focado em negócios de bairro e comunidade local

3. USO PERMITIDO

3.1 Licença de Uso
Concedemos uma licença limitada, não exclusiva e revogável para:
• Usar o aplicativo para fins pessoais e não comerciais
• Visualizar informações de negócios disponíveis
• Cadastrar negócios legítimos e verdadeiros
• Compartilhar informações de contato de estabelecimentos

3.2 Restrições de Uso
Você NÃO pode:
• Usar o aplicativo para fins ilegais ou não autorizados
• Cadastrar informações falsas ou enganosas
• Tentar hackear, modificar ou interferir no funcionamento
• Extrair dados em massa para uso comercial
• Criar contas automatizadas ou usar bots
• Violar direitos de propriedade intelectual

4. CADASTRO DE NEGÓCIOS

4.1 Responsabilidade do Usuário
• Informações cadastradas devem ser verdadeiras e atualizadas
• Usuário é responsável pela veracidade dos dados inseridos
• Não cadastrar negócios inexistentes ou ilegais
• Respeitar direitos de terceiros ao cadastrar estabelecimentos

4.2 Moderação de Conteúdo
• Reservamos o direito de remover conteúdo inadequado
• Negócios podem ser removidos sem aviso prévio
• Não garantimos a veracidade de todas as informações
• Usuários podem reportar informações incorretas

5. PROPRIEDADE INTELECTUAL

5.1 Direitos do Aplicativo
• O aplicativo e seu código são protegidos por direitos autorais
• Logomarca, design e interface são de nossa propriedade
• Não é permitida cópia, modificação ou distribuição não autorizada

5.2 Conteúdo do Usuário
• Informações de negócios cadastradas permanecem de propriedade dos respectivos donos
• Ao cadastrar, você concede licença para exibir as informações no aplicativo
• Não reivindicamos propriedade sobre dados de estabelecimentos

6. ISENÇÃO DE RESPONSABILIDADE

6.1 Uso por Sua Conta e Risco
• O aplicativo é fornecido "como está", sem garantias
• Não garantimos disponibilidade contínua do serviço
• Não somos responsáveis por danos decorrentes do uso

6.2 Informações de Terceiros
• Não verificamos todas as informações de negócios
• Não somos responsáveis por dados incorretos ou desatualizados
• Usuários devem verificar informações antes de visitar estabelecimentos
• Não endossamos ou recomendamos negócios específicos

6.3 Limitação de Responsabilidade
• Nossa responsabilidade é limitada ao máximo permitido por lei
• Não somos responsáveis por danos indiretos ou consequenciais
• Não garantimos resultados específicos do uso do aplicativo

7. PRIVACIDADE E DADOS

7.1 Coleta de Dados
• Coletamos apenas dados necessários para o funcionamento
• Dados são armazenados localmente no dispositivo
• Consulte nossa Política de Privacidade para detalhes

7.2 Proteção de Dados
• Implementamos medidas de segurança adequadas
• Não vendemos ou compartilhamos dados pessoais
• Usuários podem excluir dados desinstalando o aplicativo

8. MODIFICAÇÕES DO SERVIÇO

8.1 Atualizações
• Podemos atualizar o aplicativo periodicamente
• Novas funcionalidades podem ser adicionadas
• Algumas funcionalidades podem ser removidas ou modificadas

8.2 Descontinuação
• Reservamos o direito de descontinuar o aplicativo
• Usuários serão notificados com antecedência razoável
• Dados locais permanecerão no dispositivo após descontinuação

9. VIOLAÇÃO DOS TERMOS

9.1 Consequências
• Violações podem resultar em suspensão do acesso
• Podemos remover conteúdo inadequado
• Casos graves podem ser reportados às autoridades

9.2 Processo de Resolução
• Usuários podem contestar ações através de nossos canais
• Analisaremos cada caso individualmente
• Decisões finais são de nossa exclusiva responsabilidade

10. ALTERAÇÕES DOS TERMOS

• Estes termos podem ser atualizados periodicamente
• Alterações significativas serão notificadas no aplicativo
• Uso continuado após alterações implica aceitação
• Versão mais recente sempre disponível no aplicativo

11. LEGISLAÇÃO APLICÁVEL

• Estes termos são regidos pelas leis brasileiras
• Foro da comarca de [Cidade] para resolução de disputas
• Aplicam-se Código de Defesa do Consumidor e Marco Civil da Internet

12. CONTATO

Para dúvidas sobre estes Termos de Uso:
• Email: suporte@temakinobairro.com.br
• Endereço: [Endereço da empresa]
• Telefone: [Telefone de contato]

13. DISPOSIÇÕES GERAIS

13.1 Integralidade
• Estes termos constituem o acordo completo entre as partes
• Substituem todos os acordos anteriores sobre o mesmo assunto

13.2 Severabilidade
• Se alguma cláusula for inválida, as demais permanecem em vigor
• Cláusulas inválidas serão substituídas por outras válidas similares

13.3 Renúncia
• Não exercer um direito não constitui renúncia ao mesmo
• Renúncias devem ser expressas e por escrito

Ao usar o "Tem Aki no Bairro", você confirma ter lido, compreendido e concordado com estes Termos de Uso.
`;
